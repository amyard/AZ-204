...
{
    public DocumentSearchIndex(string id, string userId, string title, string content) : this()
    {
        Id = id;
        UserId = userId;
        Title = title;
    ...
    public DocumentSearchIndex(string id, string userId, string title, string content, DateTimeOffset modified): this(id, userId, title, content)
    {
        Modified = modified;
    }
